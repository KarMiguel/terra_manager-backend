...
export class OperacaoPlantioController extends CrudController<OperacaoPlantio, OperacaoPlantioModel> {
  ...
  @ApiOperation({
    ...
    description:
      '**Requer plano: Pro ou Premium.** Cadastra uma etapa do ciclo (tipoEtapa: PREPARO_SOLO, SEMEADURA, APLICACAO_DEFENSIVO, APLICACAO_FERTILIZANTE, IRRIGACAO, COLHEITA, OUTROS). areaHa não pode ser maior que a área plantada. custoPorHa é calculado automaticamente (custoTotal/areaHa). Opcionalmente vincula ao talhão (idTalhao). Ver REGRAS_NEGOCIO.md §7.5.',
  })
  ...
  @ApiOperation({
    ...
    description: '**Requer plano: Pro ou Premium.** Retorna operações ativas do plantio, ordenadas por dataInicio. Inclui dados do talhão quando a operação estiver vinculada a um (RN-OPE-006).',
  })

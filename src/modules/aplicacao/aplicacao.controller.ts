...
export class AplicacaoController extends CrudController<Aplicacao, AplicacaoModel> {
  ...
  @ApiOperation({
    ...
    description:
      'Cadastra aplicação com tipo (DEFENSIVO ou FERTILIZANTE), dosePorHa e unidadeDose (KG_HA, L_HA, ML_HA, etc.). quantidadeTotal é calculada automaticamente: dosePorHa × areaHa da operação (fórmula agronômica). Pode vincular a produto do estoque (idProdutosEstoque) ou informar nomeProduto. Ver REGRAS_NEGOCIO.md §7.6 e REFERENCIAS_AGRONOMIA.md.',
  })
  ...
  @ApiOperation({
    ...
    description: 'Retorna aplicações ativas da operação, ordenadas por dataAplicacao. Inclui dados do produto de estoque quando vinculado (RN-APL-005).',
  })

...
export class ZonaManejoController extends CrudController<ZonaManejo, ZonaManejoModel> {
  ...
  @ApiOperation({
    ...
    description: 'Cadastra uma zona de manejo (área com critério de manejo: fertilidade, irrigação, produtividade, etc.) com geometria GeoJSON (Polygon/MultiPolygon). Opcionalmente vincula a um talhão (idTalhao).',
  })
  ...
  @ApiOperation({
    ...
    description: 'Retorna zonas ativas da fazenda (ordenadas por nome). Suporta paginação (page, pageSize).',
  })
  ...
  @ApiOperation({
    ...
    description: 'Retorna GeoJSON FeatureCollection com as zonas de manejo da fazenda (geometria + properties: id, nome, tipo, cor, idTalhao).',
  })
  @ApiParam({ name: 'idFazenda', description: 'ID da fazenda', type: Number, example: 1 })
  @ApiResponse({
    status: 200,

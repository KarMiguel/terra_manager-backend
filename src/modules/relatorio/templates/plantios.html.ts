...
export function buildPlantiosBody(data: PlantiosTemplateData): string {
  const { plantios, areaTotal, porStatus, porCultura, porFazenda, areaMediaHa, alertas } = data;
  if (!plantios.length) {
  ...
  const resumoHtml = `
    <div class="insight-box">
      ...
      <table class="indicator-grid" style="width:100%;"><tr>
        ...
        <td><strong>Área média/plantio</strong><br>${areaMediaHa.toLocaleString('pt-BR', { minimumFractionDigits: 2 })} ha</td>
  ...
  return `
    ...
    ${alertasHtml}
    <table>
      <thead><tr><th>Fazenda</th><th>Cultivar</th><th>Cultura</th><th>Data plantio</th><th>Área (ha)</th><th>Status</th></tr></thead>
      <tbody>${rows}</tbody>
      <tfoot>
        <tr class="totais"><td colspan="4">Total</td><td>${areaTotal.toLocaleString('pt-BR')} ha</td><td>—</td></tr>
        <tr class="totais"><td colspan="6">Por status: PLANEJADO ${porStatus.PLANEJADO ?? 0} · EXECUTADO ${porStatus.EXECUTADO ?? 0} · EM_MONITORAMENTO ${porStatus.EM_MONITORAMENTO ?? 0} · CONCLUIDO ${porStatus.CONCLUIDO ?? 0}</td></tr>
      </tfoot>
    </table>`;
}
